function equalizeHeights() {
	...
	const headCells = document.querySelectorAll('.table-constructed .table__head .table__head-cell');
	let maxHeadHeight = 0;
...
function tableStickyCells() {
	...
	const $headPanel = $('.panel__head');
	const $tables = $('.table-sticked');

	$tables.each(function () {
		const $table = $(this);
		const $head = $table.find('.table__head');
		const $items = $table.find('.table__row');
		const $constructorFixed = $table.siblings('.panel__head').find('.table-constructor__fixed');


		// console.log($constructorFixed);


		if ($items.length < 1 || $head.length < 1) return;

		const isWide = $(window).width() >= 768;
		const isOverflowing = $table.get(0).scrollWidth > $table.get(0).clientWidth;
		...
		$constructorFixed.removeClass('show');

		if (isWide && isOverflowing) {
			const firstColWidth = $head.children().eq(0).outerWidth();
			...
			$constructorFixed.addClass('show');
		}
	});
}
...
function initTableConstructors() {
	const tables = document.querySelectorAll(".table-constructed");

	// console.log(tables);

	tables.forEach((table, tableIndex) => {
		
		const panelConstructor = table.closest(".panel__constructor");
		const allFieldsContainer = panelConstructor.querySelector(".table-constructor__all .table-constructor__fields");
		const fixedFieldsContainer = panelConstructor.querySelector(".table-constructor__fixed .table-constructor__fields");
		...
		function updateStickyOffsets() {
			...
			// console.log(table.scrollWidth, table.clientWidth);
			// console.log(table);
			...
			if (isOverflowing) {
				...
				if (rightFixed) {
					...
					totalRight += rightFixed.getBoundingClientRect().width;
				}
			}
			...
			stickyHeadCells.reverse().forEach((headCell, index) => {
				...
				if (index === stickyHeadCells.length - 1) {
					...
					// bodyCells.forEach(cell => cell.classList.add("table-column-sticky"));
				}
				...
				if (rightFixed) {
					...
					if (index !== -1) {
						bodyRows.forEach(row => {
							...
							// if (cell) cell.classList.add("sticky-cell");
						});
					}
				}
			}
		...
		function reorderStickyFromDOM() {
			...
			const orderedStickyIndices = Array.from(fixedFieldsContainer.querySelectorAll(".table-constructor__field"))
				...
				console.log(orderedStickyIndices);
